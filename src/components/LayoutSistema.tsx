import { BarChart3, Boxes, Building2, ChevronRight, ClipboardList, KeyRound, LogOut, Menu, Users, X } from 'lucide-react'
...
export default function LayoutSistema({children}:{children:ReactNode}){
  ...
  return <div className="app-shell">
    <aside className={`sidebar ${open?'sidebar-open':''}`}>
      <div className="brand"><div className="brand-mark"><img src="/cotapreco-icon.png" alt=""/></div><div><strong>CotaPreço</strong><span>Compras inteligentes</span></div><button className="icon-button sidebar-close" onClick={()=>setOpen(false)}><X size={20}/></button></div>
      <nav>{links.map(({to,label,icon:Icon,end})=><LinkNavegacao key={to} to={to} end={end} onClick={()=>setOpen(false)}><Icon size={20}/><span>{label}</span><ChevronRight className="nav-chevron" size={16}/></LinkNavegacao>)}{user?.role==='ADMIN'&&<><div className="nav-divider"/><span className="nav-label">Administração</span><LinkNavegacao to="/usuarios" onClick={()=>setOpen(false)}><Users size={20}/><span>Usuários</span><ChevronRight className="nav-chevron" size={16}/></LinkNavegacao><LinkNavegacao to="/dados-farmacia" onClick={()=>setOpen(false)}><Building2 size={20}/><span>Dados da farmácia</span><ChevronRight className="nav-chevron" size={16}/></LinkNavegacao></>}<div className="nav-divider"/><span className="nav-label">Minha conta</span><LinkNavegacao to="/alterar-senha" onClick={()=>setOpen(false)}><KeyRound size={20}/><span>Alterar senha</span><ChevronRight className="nav-chevron" size={16}/></LinkNavegacao></nav>
