...
import { empresaAtiva, isAdminAtivo, isAdminDoGrupo } from '../lib/permissoes'
...
export default function LayoutSistema({children}:{children:ReactNode}){
  ...
  return <div className={`app-shell ${recolhidaEfetiva?'app-shell-sidebar-recolhida':''}`}>
    <aside className={`sidebar ${open?'sidebar-open':''} ${recolhidaEfetiva?'sidebar-recolhida':''}`}>
      ...
      <div className="account"><div className="avatar">{user?.name.slice(0,2).toUpperCase()}</div><div><strong>{user?.name}</strong><span>{empresaAtiva(user)?.name}</span></div><button className="icon-button" title="Sair" onClick={()=>void logout()}><LogOut size={19}/></button></div>
    </aside>
