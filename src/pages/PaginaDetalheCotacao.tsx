import { AlertTriangle, ArrowDown, ArrowLeft, ArrowUp, CheckCircle2, Sparkles, ChevronDown, ChevronLeft, ChevronRight, ChevronUp, Clipboard, Clock3, Download, Edit3, ExternalLink, Eye, EyeOff, FileImage, FileSpreadsheet, FileText, FoldHorizontal, GripVertical, History, Link2, Lock, PackageCheck, PiggyBank, Power, RefreshCw, RotateCcw, Save, Search, Send, Share2, ShoppingCart, SlidersHorizontal, StickyNote, Trash2, Trophy, Users, X } from 'lucide-react'
...
import { ModalRaioXEconomia } from '../components/RaioXEconomia'
import { BarraFarmaciasUnificada, PainelDivisaoEstoque } from '../components/CotacaoUnificadaDetalhe'
import { perfilAtivo } from '../lib/permissoes'
...
import { LinkInterno, usarNavegacao, usarParametros, usarParametrosBusca } from '../roteamento'

type Aba='products'|'responses'|'comparison'|'purchase'
const ABAS:Aba[]=['products','responses','comparison','purchase']
const abaValida=(valor:string|null):Aba|null=>ABAS.includes(valor as Aba)?valor as Aba:null
...
/* Alguma distribuidora da divisão livre recebe mais do que o estoque que informou: permitido,
   mas exige a justificativa do que foi combinado com o representante (igual ao ajuste rápido). */
const divisaoExcedeEstoque=(produto:ComparacaoProduto,edicao:EdicaoPlano)=>edicao.allocations.some(alocacao=>{
  const oferta=produto.offers.find(o=>o.responseId===alocacao.responseId)
  return !!oferta&&alocacao.quantity>oferta.availableQuantity
})
...
export default function PaginaDetalheCotacao(){
  ...
  /* Só a cotação de demonstração pode ser apagada (o backend recusa qualquer outra), e é a
     saída de quem testou o sistema com o pedido de exemplo e agora quer a lista limpa. Some
     junto tudo o que foi pendurado nela, então a confirmação diz o que se perde: quem testou
     o fluxo inteiro pode ter proposta e pedido ali dentro sem lembrar. */
  const perdasDaDemonstracao=()=>[respostas.length&&`${respostas.length} ${respostas.length===1?'proposta recebida':'propostas recebidas'}`,pedidos.length&&`${pedidos.length} ${pedidos.length===1?'pedido gerado':'pedidos gerados'}`].filter(Boolean).join(', ')
  const excluirDemonstracao=async()=>{const perdas=perdasDaDemonstracao();if(!window.confirm(`Excluir esta cotação de demonstração?${perdas?` Junto vão: ${perdas}.`:''} Isso não pode ser desfeito.`))return;setOcupado(true);setErro('');try{await api(`/quotations/${id}`,{method:'DELETE'});navegar('/cotacoes')}catch(e){setErro(mensagemErro(e,'Não foi possível excluir a demonstração.'))}finally{setOcupado(false)}}
  ...
  /* A aba fica na URL, não só no estado: assim recarregar a página (ou voltar para um link
     salvo) reabre no mesmo lugar em vez de sempre cair em "Respostas". Troca com replace
     para não empilhar uma entrada de histórico a cada clique de aba. */
  const setAba=(proxima:Aba)=>{setAbaEstado(proxima);const proximos=new URLSearchParams(paramsBusca);proximos.set('aba',proxima);setParamsBusca(proximos,{replace:true})}
  ...
  const mensagemLink=cotacao.publicUrl?`Olá! Estamos realizando uma nova cotação.\n${cotacao.publicUrl}`:'';const podeEditar=!!user&&['ADMIN','BUYER'].includes(perfil??'')&&cotacao.status==='CLOSED';const podeGerenciar=!!user&&['ADMIN','BUYER'].includes(perfil??'')&&['DRAFT','OPEN','CLOSED'].includes(cotacao.status);const itensAtivos=cotacao.items.filter(item=>item.active)
  return <div className="page"><div className="back-row"><LinkInterno to="/cotacoes" className="text-link"><ArrowLeft/>Voltar para cotações</LinkInterno></div><div className="detail-header"><div><div className="title-line"><h1>{cotacao.name}</h1><EtiquetaStatus status={cotacao.status}/>{cotacao.demo&&<span className="badge badge-demo">Demonstração</span>}</div><p>Criada em {date(cotacao.createdAt)} · {itensAtivos.length} produtos ativos{itensAtivos.length!==cotacao.items.length?` de ${cotacao.items.length}`:''} · Prazo: {date(cotacao.expiresAt)}</p></div><div className="header-actions"><button className="button button-ghost" onClick={()=>void carregar()}><RefreshCw/>Atualizar</button>{cotacao.demo&&<button className="button button-danger-soft" disabled={ocupado} onClick={()=>void excluirDemonstracao()}><Trash2/>Excluir demonstração</button>}{cotacao.status==='DRAFT'&&<button className="button button-primary" disabled={ocupado} onClick={()=>void acao('open')}><Send/>Abrir cotação</button>}{(cotacao.status==='OPEN'||cotacao.status==='CLOSED')&&<button className="button button-secondary" disabled={ocupado} onClick={abrirProrrogacao}><Clock3/>Prorrogar</button>}{cotacao.status==='OPEN'&&<button className="button button-danger-soft" disabled={ocupado} onClick={()=>void acao('close')}><Lock/>Fechar cotação</button>}{cotacao.status==='CLOSED'&&<button className="button button-primary" disabled={ocupado} onClick={()=>void abrirConferencia()}><CheckCircle2/>Finalizar compra</button>}</div></div>{cotacao.unified&&<BarraFarmaciasUnificada vinculo={cotacao.unified} cotacaoId={cotacao.id} aba={aba}/>}{erro&&<AvisoErro message={erro}/>} {mensagem&&<div className="alert alert-success">{mensagem}</div>}{cotacao.publicUrl&&<section className="share-strip"><div><div className="share-strip-icon"><Link2/></div><div><strong>Link para representantes</strong><span>{cotacao.publicUrl}</span></div></div><div>{linkProrrogado&&<button className="button button-primary" onClick={()=>window.open(`https://wa.me/?text=${encodeURIComponent(mensagemLink)}`,'_blank','noopener,noreferrer')}><Share2/>Reenviar no WhatsApp</button>}<button className="button button-secondary" onClick={()=>void copiar(cotacao.publicUrl!,'link')}><Clipboard/>{copiado==='link'?'Copiado!':'Copiar link'}</button><button className="button button-ghost" onClick={()=>void copiar(mensagemLink,'message')}>{copiado==='message'?'Copiada!':'Copiar mensagem'}</button><a className="icon-button" title="Abrir link" href={cotacao.publicUrl} target="_blank"><ExternalLink/></a></div></section>}
...
function Comparativo({comparacao,cotacaoId,destacado}:{comparacao:ComparacaoCotacao;cotacaoId:string;destacado:number|null}){
  const[raioXAberto,setRaioXAberto]=useState(false)
  ...
  return <section className="card comparison-card"><div className="card-header"><div><h2>Comparativo de preços</h2><p>{layoutCartoes?'Cada produto vira um cartão com o melhor preço em destaque. Abra "ver ofertas" para o ranking completo de fornecedores, do mais barato ao mais caro, e arraste um fornecedor para mudar a ordem.':'Veja a faixa de preço e a economia potencial de cada produto. Segure e arraste o cabeçalho de um fornecedor para reordenar as colunas, clique no nome para ordenar por ele, e use os olhinhos para ocultar quem não interessa agora.'}</p></div>{comparacao.supplierTotals.length>1&&<button type="button" className="button button-secondary compact-action raiox-gatilho" onClick={()=>setRaioXAberto(true)}><PiggyBank/>Raio-X da economia</button>}</div>
    {raioXAberto&&<ModalRaioXEconomia comparacao={comparacao} aoFechar={()=>setRaioXAberto(false)}/>}{comparacao.supplierTotals.length===0?<EstadoVazio title="Aguardando propostas" description="O comparativo será preenchido após o primeiro envio."/>:<>
...
function CartaoComparativo({produto,fornecedoresVisiveis,ordenacaoOfertas,aberto,comDica,aoAlternar,aoReordenar,aoMover}:{produto:ComparacaoProduto;fornecedoresVisiveis:ComparacaoCotacao['supplierTotals'];ordenacaoOfertas:OrdenacaoOfertasCartao;aberto:boolean;comDica:boolean;aoAlternar:()=>void;aoReordenar:(origemId:number,destinoId:number)=>void;aoMover:(responseId:number,direcao:-1|1)=>void}){
  ...
  /* Enquanto uma linha é arrastada, as que ficam entre a origem e o destino andam uma posição
     para abrir o espaço, o que dá a sensação de encaixe ao soltar. */
  ...
  return <article className={`comparison-product-card ${aberto?'expanded':''}`}>
    <header className="comparison-product-card-head">
      <div><strong>{produto.productName}</strong><small>{produto.ean?`EAN ${produto.ean}`:'Sem EAN'}</small><TextoLaboratorio laboratory={produto.laboratory}/></div>
        ...
        <ol className="comparison-product-card-offers" id={idOfertas}>
          {ofertas.map((oferta,indice)=>{
            ...
            return <li key={oferta.responseId} data-oferta-fornecedor={oferta.responseId} className={`${posicao===1?'best':''} ${sendoArrastada?'dragging':''}`} style={deslocamento?{transform:`translateY(${deslocamento}px)`}:undefined} onPointerDown={evento=>pressionarOferta(evento,oferta.responseId)}>
              ...
              <div className="comparison-offer-supplier">
                <span className="comparison-offer-supplier-linha"><strong>{oferta.supplierName}</strong>{oferta.note&&<span title={oferta.note}><StickyNote size={13} className="comparison-offer-note-icone" aria-label={`Observação de ${oferta.supplierName}: ${oferta.note}`}/></span>}</span>
                <small>{oferta.availableQuantity} un. disponíveis</small>
                {oferta.note&&<small className="comparison-offer-note" title={oferta.note}>{oferta.note}</small>}
              </div>
...
function CompraSugerida({comparacao,pedidos,expandidas,podeEditar,podeExportar,ocupado,podeDesfazer,descricaoUltimaAlteracao,aoAlternar,aoTrocar,aoEditarPlano,aoEditarProduto,aoGerar,aoBaixar,aoBaixarRelatorio,aoCompartilhar,aoVerMinimo,aoReincluir,aoHistorico,aoDesfazer}:{comparacao:ComparacaoCotacao;pedidos:PedidoCompra[];expandidas:Set<string>;podeEditar:boolean;podeExportar:boolean;ocupado:boolean;podeDesfazer:boolean;descricaoUltimaAlteracao?:string;aoAlternar:(c:string)=>void;aoTrocar:(p:ComparacaoProduto)=>void;aoEditarPlano:()=>void;aoEditarProduto:(p:ComparacaoProduto)=>void;aoGerar:(id:number)=>void;aoBaixar:(p:PedidoCompra,f:FormatoPedido)=>void;aoBaixarRelatorio:(f:'pdf'|'excel',tipo?:TipoRelatorioPdf)=>void;aoCompartilhar:(p:PedidoCompra,f:FormatoPedido)=>void;aoVerMinimo:(id:number)=>void;aoReincluir:(id:number)=>void;aoHistorico:()=>void;aoDesfazer:()=>void}){
  ...
  return <div className="purchase-layout"><section className="card"><div className="card-header purchase-card-header"><div><h2>Plano final de compra</h2><p>Confira a divisão e gere os pedidos.</p></div><div className="purchase-header-actions">{podeExportar&&pedidos.some(pedido=>pedido.checkedAt)&&<div className="receipt-report-download" ref={menuRelatorioRef}><button type="button" className="button button-ghost compact-action download-report-trigger" disabled={ocupado} aria-label="Baixar relatório de conferência" aria-expanded={menuRelatorioAberto} aria-haspopup="menu" title="Baixar relatório de conferência" onClick={()=>{setFormatoRelatorioSelecionado(null);setMenuRelatorioAberto(aberto=>!aberto)}}><Download/><span>Baixar relatório</span><ChevronDown/></button>{menuRelatorioAberto&&<div className="receipt-report-menu" role="menu">{formatoRelatorioSelecionado?<><strong className="receipt-report-menu-title">Qual relatório em {formatoRelatorioSelecionado==='pdf'?'PDF':'Excel'}?</strong><button type="button" role="menuitem" onClick={()=>{setMenuRelatorioAberto(false);setFormatoRelatorioSelecionado(null);aoBaixarRelatorio(formatoRelatorioSelecionado,'general')}}>{formatoRelatorioSelecionado==='pdf'?<FileText/>:<FileSpreadsheet/>}Relatório geral da cotação</button><button type="button" role="menuitem" onClick={()=>{setMenuRelatorioAberto(false);setFormatoRelatorioSelecionado(null);aoBaixarRelatorio(formatoRelatorioSelecionado,'divergences')}}><AlertTriangle/>Somente divergências</button></>:<><button type="button" role="menuitem" onClick={()=>setFormatoRelatorioSelecionado('pdf')}><FileText/>PDF</button><button type="button" role="menuitem" onClick={()=>setFormatoRelatorioSelecionado('excel')}><FileSpreadsheet/>Excel</button></>}</div>}</div>}{podeEditar&&podeDesfazer&&<button className="button button-ghost compact-action undo-plan-trigger" disabled={ocupado} aria-label="Desfazer última alteração do plano" title={descricaoUltimaAlteracao?`Desfazer: ${descricaoUltimaAlteracao}`:'Desfazer última alteração'} onClick={aoDesfazer}><RotateCcw/><span>Desfazer</span></button>}{podeEditar&&<><button className="button button-ghost compact-action history-plan-trigger" aria-label="Histórico do plano" title="Histórico do plano" onClick={aoHistorico}><History/><span>Histórico do plano</span></button><button className="button button-secondary compact-action review-plan-trigger" aria-label="Revisar todos os produtos" title="Revisar todos os produtos" onClick={aoEditarPlano}><Edit3/><span>Revisar todos os produtos</span></button></>}</div></div>
    ...
    </section><aside className="purchase-summary"><div className="summary-highlight"><ShoppingCart/><span>Compra sugerida</span><strong>{money(comparacao.bestCompositionTotal)}</strong></div><button type="button" className="summary-economia" title="Ver de onde vem a economia: seu plano contra o 2º, o 3º e o preço mais alto" onClick={()=>setRaioXAberto(true)}><span>Economia estimada</span><strong>{money(comparacao.estimatedSavings)}</strong><small><PiggyBank/>Ver raio-x da economia</small></button><div className={abaixo>0?'summary-warning':''}><span>Pedidos abaixo do mínimo</span><strong>{abaixo}</strong></div><div><span>Produtos sem cobertura</span><strong>{comparacao.productsWithoutOffer+comparacao.partiallyCoveredProducts}</strong></div></aside>
    {raioXAberto&&<ModalRaioXEconomia comparacao={comparacao} aoFechar={()=>setRaioXAberto(false)}/>}</div>
}
...
/* Laboratório aparece por extenso: é mais útil para quem está comprando do que ficar
   escondido atrás de um ícone. Sem className extra o small já herda o estilo padrão de
   cada tela onde aparece. */
function TextoLaboratorio({laboratory}:{laboratory:string|null}){
  return laboratory?<small className="produto-laboratorio">{laboratory}</small>:null
}
...
function MatrizDistribuidoras({produtos,edicoes,erros,alterar}:{produtos:ComparacaoProduto[];edicoes:EdicaoPlano[];erros:Record<string,string>;alterar:(itemId:number,parcial:Partial<EdicaoPlano>)=>void}){
  ...
  <div className="table-wrap matriz-wrap" ref={area} onScroll={()=>espelhar(area.current,barra.current)}><table className="matriz-plano" id="matriz-plano">
    ...
    <tbody>{produtos.map(produto=>{
      ...
      {divisaoExcedeEstoque(produto,edicao)&&<tr className="matriz-justificativa-linha">
        <td colSpan={colunas.length+2}>
          <div className="plan-note guided-stock-note"><div><strong>Confirmação de estoque necessária</strong><span>{produto.offers.filter(oferta=>(edicao.allocations.find(a=>a.responseId===oferta.responseId)?.quantity??0)>oferta.availableQuantity).map(oferta=>`${oferta.supplierName} informou ${oferta.availableQuantity} un., você quer ${edicao.allocations.find(a=>a.responseId===oferta.responseId)?.quantity} un.`).join(' · ')}. Explique o que foi combinado com o representante.</span></div>
            <label>Justificativa interna<input placeholder="Ex.: representante confirmou mais 5 unidades por telefone" value={edicao.stockOverrideNote}
              aria-label={`Justificativa do estoque adicional de ${produto.productName}`}
              onChange={evento=>alterar(produto.quotationItemId,{stockOverrideNote:evento.target.value})}/>
              <small>Esta informação não aparecerá no pedido enviado.</small>
              {!edicao.stockOverrideNote.trim()&&<small className="field-error">Preencha a justificativa para salvar.</small>}
              {erros[`${prefixo}stockOverrideNote`]&&<small className="field-error">{erros[`${prefixo}stockOverrideNote`]}</small>}</label>
          </div>
        </td>
      </tr>}
