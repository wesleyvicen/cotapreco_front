...
function CompraSugerida({comparacao,pedidos,expandidas,podeEditar,podeExportar,ocupado,aoAlternar,aoTrocar,aoEditarPlano,aoEditarProduto,aoGerar,aoBaixar,aoBaixarRelatorio,aoCompartilhar,aoVerMinimo,aoReincluir,aoHistorico}:{comparacao:ComparacaoCotacao;pedidos:PedidoCompra[];expandidas:Set<string>;podeEditar:boolean;podeExportar:boolean;ocupado:boolean;aoAlternar:(c:string)=>void;aoTrocar:(p:ComparacaoProduto)=>void;aoEditarPlano:()=>void;aoEditarProduto:(p:ComparacaoProduto)=>void;aoGerar:(id:number)=>void;aoBaixar:(p:PedidoCompra,f:FormatoPedido)=>void;aoBaixarRelatorio:(f:'pdf'|'excel',tipo?:TipoRelatorioPdf)=>void;aoCompartilhar:(p:PedidoCompra,f:FormatoPedido)=>void;aoVerMinimo:(id:number)=>void;aoReincluir:(id:number)=>void;aoHistorico:()=>void}){
  ...
  const recolherFornecedor=(chave:string,botao:HTMLButtonElement)=>{
    const cabecalho=botao.closest<HTMLElement>('.supplier-purchase-card')?.querySelector<HTMLButtonElement>('.supplier-toggle')
    aoAlternar(chave)
