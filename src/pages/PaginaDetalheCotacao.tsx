...
function ModalPlano({produtos,edicoes,setEdicoes,erros,ocupado,focado,aoFechar,aoSalvar}:{produtos:ComparacaoProduto[];edicoes:EdicaoPlano[];setEdicoes:(e:EdicaoPlano[])=>void;erros:Record<string,string>;ocupado:boolean;focado:boolean;aoFechar:()=>void;aoSalvar:()=>void}){
  ...
  return <div className="modal-backdrop"><section className={`modal purchase-plan-modal ${focado?'focused-plan-modal':''}`}>
    ...
    <div className={`plan-items ${modoEdicao==='avancado'?'avancado':''}`}>{produtosVisiveis.length>0&&(modoEdicao==='rapido'
      ...
      {modoEdicao==='rapido'?<><div className="guided-fields quick-guided-fields">
        ...
        :<p className="plan-resultado neutro">{previsao.parcelas.length>1?`Dividido hoje entre ${previsao.parcelas.length} distribuidoras`:'Sem alteração'}</p>}</>:<div className="guided-fields">
        <div className="guided-field guided-field-quantidade"><label className="sr-only" htmlFor={`quantidade-final-${p.quotationItemId}`}>Total de {p.productName}</label><input id={`quantidade-final-${p.quotationItemId}`} type="number" min="0" value={e.desiredQuantity} onChange={x=>{const quantidade=Math.max(0,Number(x.target.value));alterar(p.quotationItemId,{desiredQuantity:quantidade,championQuantity:e.championQuantity!=null?Math.min(e.championQuantity,quantidade):null})}}/>{e.desiredQuantity===0&&<span className="guided-hint warning">Sai da compra</span>}</div>
        <div className="guided-field"><label className="sr-only" htmlFor={`distribuidora-${p.quotationItemId}`}>Comprar {p.productName} primeiro de</label><select id={`distribuidora-${p.quotationItemId}`} value={automatico?'':e.selectedResponseId??''} disabled={p.offers.length===0||e.desiredQuantity===0} onChange={x=>{if(!x.target.value){alterar(p.quotationItemId,{selectedResponseId:p.offers[0]?.responseId??null,manualSelection:false,championQuantity:null,stockOverrideNote:''});return}const id=Number(x.target.value);const selecionada=p.offers.find(o=>o.responseId===id);alterar(p.quotationItemId,{selectedResponseId:id,manualSelection:true,championQuantity:Math.min(e.desiredQuantity,selecionada?.availableQuantity??e.desiredQuantity),stockOverrideNote:''})}}><option value="">Automático — mais baratas</option>{p.offers.map(o=><option key={o.responseId} value={o.responseId}>{o.supplierName} — {money(o.unitPrice)} · estoque {o.availableQuantity} un.</option>)}</select>{p.offers.length===0&&<span className="guided-hint warning">Sem oferta</span>}</div>
        <div className={`guided-field guided-field-quantidade ${automatico?'guided-field-disabled':''}`}><label className="sr-only" htmlFor={`quantidade-principal-${p.quotationItemId}`}>Quantidade de {p.productName} nesta distribuidora</label><input id={`quantidade-principal-${p.quotationItemId}`} type="number" min="1" max={e.desiredQuantity} disabled={automatico||!e.selectedResponseId||e.desiredQuantity===0} value={automatico?'':e.championQuantity??''} placeholder={automatico?'Automático':'Qtd.'} onChange={x=>alterar(p.quotationItemId,{championQuantity:x.target.value?Number(x.target.value):null})}/>{!automatico&&oferta&&<div className="quantity-shortcuts"><button type="button" title={`Usar o estoque informado por ${oferta.supplierName}`} onClick={()=>alterar(p.quotationItemId,{championQuantity:Math.min(e.desiredQuantity,oferta.availableQuantity),stockOverrideNote:''})}>Estoque ({Math.min(e.desiredQuantity,oferta.availableQuantity)})</button><button type="button" onClick={()=>alterar(p.quotationItemId,{championQuantity:e.desiredQuantity})}>Tudo dela</button></div>}{acimaDoTotal&&<small className="field-error">Essa quantidade não pode ser maior que o total da compra.</small>}{erros[`${prefixo}championQuantity`]&&<small className="field-error">{erros[`${prefixo}championQuantity`]}</small>}</div>
      </div>}
      ...
      {/* Só no avançado: no rápido tudo vai para uma distribuidora, e a linha de resultado
          acima já diz qual. Repetir a divisão em bloco fazia um produto ocupar a tela toda. */}
      {modoEdicao==='avancado'&&<div className={`allocation-preview ${previsao.restante>0?'allocation-preview-warning':''}`}>
        <div className="allocation-preview-title"><strong>Como ficará</strong><span>{previsao.automatico?'automático':'escolhido por você'}</span></div>
        {e.desiredQuantity===0?<p>Sai do plano de compra.</p>:previsao.parcelas.length===0?<p>Sem oferta disponível.</p>:<>
          <div className="allocation-preview-lines">{previsao.parcelas.map(parcela=><div key={`${parcela.supplierName}-${parcela.position}`}><span>{parcela.quantity} un.</span><strong>{parcela.supplierName}</strong><small>{parcela.principal?(previsao.automatico?'melhor oferta':'1ª escolhida'):`complemento · ${parcela.position}ª`}</small></div>)}</div>
          {previsao.restante>0&&<p>Faltam <strong>{previsao.restante} un.</strong> por falta de estoque nas propostas.</p>}
        </>}
      </div>}
    </article>})}</div>
