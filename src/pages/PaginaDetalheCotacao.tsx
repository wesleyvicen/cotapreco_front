import { AlertTriangle, ArrowDown, ArrowLeft, ArrowUp, CheckCircle2, Sparkles, ChevronDown, ChevronLeft, ChevronRight, ChevronUp, Clipboard, Clock3, Download, Edit3, ExternalLink, Eye, EyeOff, FileImage, FileSpreadsheet, FileText, FlaskConical, FoldHorizontal, GripVertical, History, Link2, Lock, PackageCheck, Power, RefreshCw, RotateCcw, Save, Search, Send, Share2, ShoppingCart, SlidersHorizontal, StickyNote, Trophy, Users, X } from 'lucide-react'
...
function CartaoComparativo({produto,fornecedoresVisiveis,ordenacaoOfertas,aberto,comDica,aoAlternar,aoReordenar,aoMover}:{produto:ComparacaoProduto;fornecedoresVisiveis:ComparacaoCotacao['supplierTotals'];ordenacaoOfertas:OrdenacaoOfertasCartao;aberto:boolean;comDica:boolean;aoAlternar:()=>void;aoReordenar:(origemId:number,destinoId:number)=>void;aoMover:(responseId:number,direcao:-1|1)=>void}){
  ...
  return <article className={`comparison-product-card ${aberto?'expanded':''}`}>
    <header className="comparison-product-card-head">
      <div><strong>{produto.productName}</strong><IndicadorLaboratorio laboratory={produto.laboratory}/><small>{produto.ean?`EAN ${produto.ean}`:'Sem EAN'}</small></div>
...
/* Ícone em vez de linha extra: laboratório é informação secundária, e mostrar sempre por
   extenso deixaria toda lista mais alta. O title nativo também fica de reforço (leitor de
   tela, e quem prefere só passar o mouse e esperar), mas o balão no clique é o caminho
   principal — o delay do tooltip do navegador para aparecer é longo demais para uma lista
   onde a pessoa passa o olho em vários produtos seguidos. */
function IndicadorLaboratorio({laboratory}:{laboratory:string|null}){
  const [aberto,setAberto]=useState(false)
  const ref=useRef<HTMLSpanElement>(null)
  useEffect(()=>{
    if(!aberto)return
    const fechar=(evento:MouseEvent)=>{if(!ref.current?.contains(evento.target as Node))setAberto(false)}
    document.addEventListener('mousedown',fechar)
    return()=>document.removeEventListener('mousedown',fechar)
  },[aberto])
  if(!laboratory)return null
  return <span ref={ref} className="laboratorio-indicador" title={laboratory} aria-label={`Laboratório: ${laboratory}`}
    onClick={evento=>{evento.stopPropagation();setAberto(atual=>!atual)}}>
    <FlaskConical/>
    {aberto&&<span className="laboratorio-bolha" role="tooltip">{laboratory}</span>}
  </span>
}
  ...
  <div className="table-wrap matriz-wrap" ref={area} onScroll={()=>espelhar(area.current,barra.current)}><table className="matriz-plano" id="matriz-plano">
    ...
    <tbody>{produtos.map(produto=>{
      ...
      return <tr key={produto.quotationItemId} ref={produto.quotationItemId===primeiroRecusado?primeiraComErro:undefined} className={erroLinha?'matriz-linha-erro':''}>
        <th scope="row"><strong>{produto.productName}<SeloRecebido produto={produto}/><IndicadorLaboratorio laboratory={produto.laboratory}/></strong><span>pedido {produto.requestedQuantity} un.</span></th>
        {colunas.map(coluna=>{
...
function ModalPlano({produtos,edicoes,setEdicoes,erros,erroGeral,ocupado,focado,aoFechar,aoSalvar}:{produtos:ComparacaoProduto[];edicoes:EdicaoPlano[];setEdicoes:(e:EdicaoPlano[])=>void;erros:Record<string,string>;erroGeral:string;ocupado:boolean;focado:boolean;aoFechar:()=>void;aoSalvar:()=>void}){
  ...
  return <div className="modal-backdrop"><section className={`modal purchase-plan-modal ${focado?'focused-plan-modal':''}`}>
    ...
    {modoEdicao==='avancado'
      ...
      <div className="plan-product"><strong>{p.productName}<SeloRecebido produto={p}/><IndicadorLaboratorio laboratory={p.laboratory}/></strong><span>Pedido original: {p.requestedQuantity} {p.requestedQuantity===1?'unidade':'unidades'}</span></div>
...
function ModalTroca({produto,ocupado,aoFechar,aoEscolher,aoAutomatico}:{produto:ComparacaoProduto;ocupado:boolean;aoFechar:()=>void;aoEscolher:(id:number)=>void;aoAutomatico:()=>void}){
  return <div className="modal-backdrop"><section className="modal winner-modal">
    <div className="modal-header modal-header-simple"><div><h2>Trocar campeão</h2><div className="winner-product-title"><PackageCheck/><span>{produto.productName}</span><IndicadorLaboratorio laboratory={produto.laboratory}/></div></div><button className="icon-button" aria-label="Fechar" onClick={aoFechar}><X/></button></div>
